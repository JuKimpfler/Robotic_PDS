...
groups you've already customized, and keeps a separate descriptor per node
so switching nodes restores the right names instantly. See
`Doku/Kanalnamen_Implementierung.md` for the full protocol and
...
- **`teensy_firmware/`**: PlatformIO project + the drop-in Teensy library — see [`teensy_firmware/README.md`](teensy_firmware/README.md) for the full API.
  ...
  - `src/channel_config.h`: **optional** — param names and the graphic-overlay mapping (see Architecture Overview, section 3). Debug-channel names are easier to set in the sketch via `PDS.plot()`/`PDS.track()`.
...
- **`tools/`**: developer checks that need no hardware.
  - `selftest.py`: frame assemblers, descriptor reassembly, param I/O, Bluetooth protocol — 46 assertions, standard library only.
  - `check_wire_format.py`: verifies that Teensy, node and GUI agree on every magic number and packet size.
  - `build_teensy_check.sh`: compiles the Teensy library in three configurations using the toolchain PlatformIO already installed.
...
Three scripts cover everything that can be verified without a Teensy, a node
or a Wi-Fi link. Run them before committing firmware or protocol changes:
